...
class Ecosystem:
    ...
    def _pair_mature(self, species_id: str, pressure: EnvironmentalPressure | None = None) -> list[tuple[Organism, Organism]]:
        ...
        if pressure is None:
            ...
            mature.sort(key=lambda organism: (-(adaptive_fitness(organism.genome, pressure) if organism.genome is not None else 0.0), organism.organism_id))
        return list(zip(mature[::2], mature[1::2]))

    def step(self, ticks: int = 1, environmental_stress: float = 0.0, selection_pressure: Mapping[str, EnvironmentalPressure] | None = None) -> None:
        if ticks < 0:
