...
class PlanetEcologyRuntime:
    """Apply planetary habitat conditions to organisms and track terrestrial biomass."""
    ...
    def habitat_stress(self, snapshot: PlanetSnapshot, organism: Organism) -> float:
        ...
        x = max(0, min(max_x, round(organism.position.x)))
        y = max(0, min(max_y, round(organism.position.z)))
        cell = snapshot.cells[y][x]
        preferred_temperature = 15.0
        thermal = min(1.0, abs(cell.atmosphere.temperature_c - preferred_temperature) / 35.0)
        ...
        return min(1.0, 0.5 * thermal + 0.3 * water_stress + 0.2 * productivity_stress)
    ...
    def apply_to_ecosystem(self, ecosystem: Ecosystem, snapshot: PlanetSnapshot) -> None:
        for organism in ecosystem.organisms.values():
            if organism.alive:
                organism.survive(self.habitat_stress(snapshot, organism))
