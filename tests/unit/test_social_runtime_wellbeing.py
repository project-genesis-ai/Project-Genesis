...
def test_conflict_adds_bounded_social_stress() -> None:
    ...
    agents = {
        "a": Agent("a", "A", personality=hostile, needs=Needs(social=0.2)),
        "b": Agent("b", "B", personality=opposite, needs=Needs(social=0.2)),
    }

    SocialRuntime().step(SocialSystem(), agents, tick=1)

    assert 0.2 <= agents["a"].needs.social <= 1.0
    assert 0.2 <= agents["b"].needs.social <= 1.0
...
def test_social_runtime_rejects_invalid_parameters() -> None:
    try:
        ...
        raise AssertionError("zero interaction budget must be rejected")
    ...
    try:
        ...
        raise AssertionError("negative relief must be rejected")
